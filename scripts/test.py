...
def save_images(dataframe, folder, num_threads=60):
    real_images = 0
...
def create_datasets(num_train_images):
    ...
    num_val_images = 4800 #max(1, num_train_images // 10)  # Ensure at least one image if num_train_images is low
    val_samples = 0
    val_data_selected = []
    ...
    for batch in val_data.iter(batch_size=batch_size):
        ...
        if val_data_selected:
            ...
            save_images(val_df, os.path.join(output_dir, "test"))
    ...
    for batch in loaded_data.iter(batch_size=batch_size, start=batch_count):
        ...
        if test_data:
            ...
            save_images(df, os.path.join(output_dir, "val"))
...
if __name__ == "__main__":
    num_train_images = 2306629  # Set this to the desired number of training images
