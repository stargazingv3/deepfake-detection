...
def calculate_deepfake_percentage(num_samples):
    ...
    def is_deepfake(row):
        # Check if any model_gen or filepath_gen field is present
        return any(row.get(f'model_gen{i}') for i in range(4)) or any(row.get(f'filepath_gen{i}') for i in range(4))
